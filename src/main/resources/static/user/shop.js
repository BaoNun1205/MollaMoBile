    ...
    function filterProducts(minPrice, maxPrice, categoryIds, pageNo = 1) {
        ...
        fetch('/shop/filter-products', {
            ...
            body: JSON.stringify({ filters: selectedFilters, minPrice: minPrice, maxPrice: maxPrice, categoryIds: categoryIds, keyword: keyword, pageNo: pageNo })
        })
            ...
            .then(data => {
                ...
                // Cập nhật phân trang
                updatePagination(data.currentPage, data.totalPages, minPrice, maxPrice, categoryIds);
            })
            .catch(error => console.error('Error:', error));
    }

    // Cập nhật phân trang
    function updatePagination(currentPage, totalPages, minPrice, maxPrice, categoryIds) {
        ...
        ${totalPages > 0 ? `
            <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                <a class="page-link page-link-prev" href="#" ${currentPage > 1 ? `onclick="filterProducts(${minPrice}, ${maxPrice}, ${JSON.stringify(categoryIds)}, ${currentPage - 1}); return false;"` : 'aria-disabled="true"'} aria-label="Previous">
        ...
        ${Array.from({ length: totalPages }, (_, i) => i + 1).map(i => `
            <li class="page-item ${currentPage == i ? 'active' : ''}">
                <a class="page-link" href="#" onclick="filterProducts(${minPrice}, ${maxPrice}, ${JSON.stringify(categoryIds)}, ${i}); return false;">${i}</a>
            </li>
        `).join('')}
        ${totalPages > 0 ? `
            <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                <a class="page-link page-link-prev" href="#" ${currentPage < totalPages ? `onclick="filterProducts(${minPrice}, ${maxPrice}, ${JSON.stringify(categoryIds)}, ${currentPage + 1}); return false;"` : 'aria-disabled="true"'} aria-label="Next">
    ...
    function initializePriceSlider(minPrice = 0, maxPrice = 2000) {
        ...
        priceSlider.noUiSlider.on('update', function(values, handle) {
            ...
            if (categoryIds.length > 0){
                filterProducts(currentMinPrice, currentMaxPrice, categoryIds);
            } else {
                filterProducts(currentMinPrice, currentMaxPrice, null);
            }
    ...
    function handleCheckboxChange() {
        ...
        if (isAnyPriceCheckboxChecked && !isAnyCategoryCheckboxChecked) {
            ...
            filterProducts(null, null, null); // min, max của slider là null, categoryIds là null
        }
        // Nếu không có checkbox giá và có checkbox danh mục
        else if (!isAnyPriceCheckboxChecked && isAnyCategoryCheckboxChecked) {
            // Gửi yêu cầu AJAX với giá trị mặc định của slider và các bộ lọc checkbox giá hiện tại
            filterProducts(minPrice, maxPrice, categoryIds); // min, max của slider là null, categoryIds là null
        }
        // Nếu cả hai đều có
        else if (isAnyPriceCheckboxChecked && isAnyCategoryCheckboxChecked) {
            // Đặt lại thanh slider về giá trị mặc định
            ...
            filterProducts(null, null, categoryIds); // min, max của slider là null
        }
        // Nếu cả hai đều không có
        else {
            // Lọc theo slider
            if (priceSlider && priceSlider.noUiSlider) {
                filterProducts(minPrice, maxPrice, null);
            }
        }
